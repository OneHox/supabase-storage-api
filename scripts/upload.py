...
if not (os.path.exists(src)):
  ...
  supabase.storage.from_(bkt).remove(f'{pth}/{b64}')
  supabase.storage.from_(bkt).upload(file=b64, path=f'{pth}/{b64}', file_options={"content-type": "text/plain", "x-upsert": "true", "cache-control": "3600"})
  clean()
